...
class ShopCart extends Component {
  ...
  render() {
    ...
    // 全选框是否勾选
    const checkAllValue = Object.keys(selectedMap).every(
      key => selectedMap[key]
    );

    return (
      <div className="shop-cart">
        <header className="shop-cart-header">购物车</header>
        <section className="policys-wrapper">
          <PolicyList
            policys={[
              '30天无忧退货',
              '48小时快速退款',
              `满${discountPrice}元免邮费`
            ]}
          />
        </section>
        <section className="discount">
          ...
          {discountDiff > 0 ? (
            <span>
              {' '}
              再购<span className="diff">{discountDiff}</span>元立享满包邮
            </span>
          ) : (
            '已满足包邮条件'
          )}
          ...
          <div>
              ...
              {goods.map((good, index) => (
                <div className="good-item" key={index}>
                  ...
                  <div className="control">
                    <CartControl
                      onAdd={e => this.handleControlEvent('ADD', index, e)}
                      onDecrease={e =>
                        this.handleControlEvent('DECREASE', index)
                      }
                      value={good.number}
                    />
                  </div>
            ...
            <section className="accounts-bar">
              ...
              <div className="pay-wrapper">
                ...
                <div className={payCls} onClick={this.pay}>
                  下单
                </div>
              </div>
            </section>
